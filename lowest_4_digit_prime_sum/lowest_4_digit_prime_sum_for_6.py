...
valid_set = [0] * 3
test_set = [0] * 3
prime_set = list(primerange(1, 300)) 
prime_set.remove(5)
len_prime_set = len(prime_set)
prime_set_last_value = prime_set[len_prime_set-1]
smalest_iterator = 2
milisec_iterator = second_iterator = minute_iterator = hour_iterator = day_iterator = week_iterator = smalest_iterator
...
while verify_if_matches(test_set) == False:
    ...
    while verify_if_matches(test_set[:5]) == False ^ restart:
        second_iterator += 1
        test_set[4] = prime_set[second_iterator]
        if test_set[4] == prime_set[len_prime_set-1]:
            ...
            if test_set[3] != prime_set[len_prime_set-1]:
                ...
                restart = True
    milisec_iterator = second_iterator
    test_set[5] = prime_set[milisec_iterator]
    while verify_if_matches(test_set) == False ^ restart:
        milisec_iterator += 1
        test_set[5] = prime_set[milisec_iterator]
        if test_set[5] == prime_set[len_prime_set-1]:
            # second_iterator = smalest_iterator
            # minute_iterator += 1
            # test_set[4] = test_set[smalest_iterator]
            if test_set[3] != prime_set[len_prime_set-1]:
